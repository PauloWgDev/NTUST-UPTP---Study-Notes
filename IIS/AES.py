...
#SubBytes is a bricklayer permutation consisting of an S-box applied to the bytes of the state
...
def invers_subBytes(s):
    for i in range(4):
        for j in range(4):
            s[i][j] = invers_s_box[s[i][j]]
...
Poly_time = lambda a: (((a << 1) ^ 0x1B) & 0xFF) if (a & 0x80) else (a << 1)

# Efficient implementation of MixColumns.
def mix_single_column(a): # column

    t = a[0] ^ a[1] ^ a[2] ^ a[3]
    u = a[0]
    a[0] ^= t ^ Poly_time(a[0] ^ a[1])
    a[1] ^= t ^ Poly_time(a[1] ^ a[2])
    a[2] ^= t ^ Poly_time(a[2] ^ a[3])
    a[3] ^= t ^ Poly_time(a[3] ^ u)
...
def inv_mix_columns(s):
    ...
    for i in range(4):
        u = Poly_time(Poly_time(s[i][0] ^ s[i][2]))
        v = Poly_time(Poly_time(s[i][1] ^ s[i][3]))
        s[i][0] ^= u
        s[i][1] ^= v
        s[i][2] ^= u
        s[i][3] ^= v
...
class AES:
    ...
    def _expand_key(self, master_key):
        ...
        while len(key_columns) < (self.n_rounds + 1) * 4:
            ...
            if len(key_columns) % iteration_size == 0:
                ...
                word[0] ^= r_con[i]
                i += 1
            elif len(master_key) == 16 and len(key_columns) % iteration_size == 4:
    ...
    def decrypt_block(self, ciphertext):
        ...
        invers_shift_rows(cipher_state)
        invers_subBytes(cipher_state)
    ...
    def decrypt_cbc(self, ciphertext, iv):
        ...
        return unpad(b''.join(blocks)).decode('latin-1')
    ...
    def encrypt_cfb(self, plaintext, iv):
        ...
        for plaintext_block in split_blocks(plaintext, require_padding=False):
            ...
            ciphertext_block = xorBytes(plaintext_block, self.encrypt_block(prev_ciphertext))
    ...
    def decrypt_cfb(self, ciphertext, iv):
        ...
        for ciphertext_block in split_blocks(ciphertext, require_padding=False):
            ...
            plaintext_block = xorBytes(ciphertext_block, self.encrypt_block(prev_ciphertext))
...
def test():
    ...
    key = os.urandom(32)
    iv = os.urandom(16)
    encrypted = AES(key).encrypt_cbc(b'Hello NTUST', iv)
    print('    ')
    print('<=====================>')
    ...
    print('    ')
    print('<=====================>')
    plaintext = AES(key).decrypt_cbc(encrypted, iv).decode()
